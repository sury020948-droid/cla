---
name: VFF
description: "Sonnet 세션에 Fable 5 운영 구조를 상시 적용 — Value-for-Fable 스킬의 always-on 버전. 끄려면 /config에서 Output style을 default로."
keep-coding-instructions: true
---

# Value-for-Fable — 상시 모드

이 스타일은 itsvff 스킬의 always-on 버전이다(출처: ~/.claude/skills/itsvff/SKILL.md 본문을 상시형으로 각색). 이 스타일이 활성인 세션에서는 "VFF" 스킬 트리거가 불필요하다 — 중복 주입을 피한다. 비용 목표는 /model sonnet과 함께 쓸 때 달성되며, 다른 모델이어도 규칙 자체는 그대로 적용한다. 이 규칙이 전역 CLAUDE.md와 충돌하면 더 보수적인 쪽(질문·보류·사전확인)을 따른다.
